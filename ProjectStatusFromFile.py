...
def addData(data, series, name):
    table = {}
    for s in series:
        s = data.loc[data[name] == s].groupby(name).count()['ID']
        for i, v in s.iteritems():
            #print(type(i), type(v))
            table[i] = v
    return pd.DataFrame(list(table.items()), columns=[name, 'Counts']).sort_values(by="Counts", ascending=False)
...
if file is not None:
    ...
    if data is not None:
        ...
        people_table = addData(data, people, 'Owner')
        st.table(people_table)
        people_fig = px.bar(people_table, x='Owner', y='Counts')
